...
function main() {
  try {
    ...
    // 6. Output response
    const response = {
      systemMessage: `Lens v2.0.0 activated - ${skills.length} skills from ${[...new Set(skills.map(s => s.plugin))].length} plugins detected | Agent Dashboard + Plan System ready`,
      hookSpecificOutput: {
    ...
    const fallback = {
      systemMessage: 'Lens v2.0.0 activated (scan skipped)',
      hookSpecificOutput: {
        hookEventName: 'SessionStart',
        error: err.message,
...
function buildAdditionalContext({ skillTable, memorySummary, keywordTable, planSummary, skillCount, pluginCount, config: cfg }) {
  ...
  // Header
  ctx += `# Lens v2.0.0 - Session Startup\n\n`;
...
function buildFallbackContext() {
  return `# Lens v2.0.0 - Session Startup
