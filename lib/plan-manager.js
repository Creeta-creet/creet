 ...
 * Priority: config override → project docs/ → .creet/plans/ (fallback)
...
function savePlanState(plan) {
  ...
  try {
    ...
    console.error(`[creet] savePlanState failed: ${err.message}`);
...
function loadPlanState(planId) {
  ...
  try {
    ...
    console.error(`[creet] loadPlanState failed: ${err.message}`);
  }
  return planId ? null : {};
}
...
function generatePlanContent(planData) {
  ...
  content += `> Generated by Creet Plan on ${dateStr}\n`;
...
function parsePlanFrontmatter(filePath) {
  try {
    ...
    console.error(`[creet] parsePlanFrontmatter failed: ${err.message}`);
    return null;
  }
}
 ...
 * @param {string} newStatus - New status (draft|approved|executing|completed|failed)
...
function updatePlanStatus(filePath, newStatus) {
  ...
  try {
    ...
    const frontmatter = parsePlanFrontmatter(filePath);
    if (frontmatter && frontmatter.id) {
      const state = loadPlanState(frontmatter.id);
      if (state) {
        state.status = newStatus;
        if (newStatus === 'approved') state.approvedAt = new Date().toISOString();
        if (newStatus === 'completed' || newStatus === 'failed') state.completedAt = new Date().toISOString();
        savePlanState(state);
      }
    }
    ...
    console.error(`[creet] updatePlanStatus failed: ${err.message}`);
    return { success: false, error: err.message };
  }
}
